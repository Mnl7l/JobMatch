    ...
    if (selectedFile) {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          setMessage({ text: 'Error analyzing resume with AI', type: 'error' });
        } finally {
      ...
      if (selectedFile.type === 'application/pdf') {
        ...
        // This is a simplified example - you might need a PDF parsing library
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'upload' && file) {
        ...
        resumeDataToSave.file_url = publicUrl;
        
        // Parse some basic information from filename if needed
        resumeDataToSave.original_filename = file.name;
        
        // If we have AI analysis, store it as well
        if (aiSuggestions) {
          resumeDataToSave.ai_analysis = JSON.stringify(aiSuggestions);
        }
      } else if (mode === 'manual') {
        // Handle manual entry
        resumeDataToSave = {
